...
def test_next_dollar_table_ranks_every_continuous_input():
...
def test_ltcg_stacking_table_reproduces_the_marginal_rate_hump():
    """Gain stacking makes the wage rate rise to 27%, then fall to 22%."""
    table = EXAMPLE["ltcg_stacking_table"]()

    assert table["next_wage_dollar_percent"].to_list() == pytest.approx(
        [27.0, 27.0, 27.0, 22.0, 22.0],
        abs=1e-7,
    )
    assert table["total_tax"].to_list() == pytest.approx(
        [4_372.25, 9_772.25, 12_472.25, 13_641.0, 14_741.0],
        abs=0.01,
    )


def test_derivative_guided_sizing_is_more_precise_with_fewer_evaluations():
    ...
    assert derivative.evaluations < 40
    assert forward_guided.ordinary_income == pytest.approx(35_125.0, abs=0.01)
    assert forward_guided.evaluations == derivative.evaluations * 2
