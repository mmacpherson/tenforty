...
produces a quiet agreement, because the oracle is answering a different
...
if not os.environ.get("TENFORTY_ORACLE"):
    pytest.skip(
        "oracle adapter tests require the oracle dependency group; "
        "set TENFORTY_ORACLE=1 to run",
...
from .oracle_policy import evaluate_components
...
def test_iso_changes_the_oracle_result():
    ...
    assert with_iso != pytest.approx(without_iso, abs=1.0), (
        "iso=200,000 and iso=0 produced the same oracle result"
    )


def test_case_fields_all_reach_the_adapter():
    """Every numeric field of a case must change some oracle output.
...
def test_oracle_amt_matches_form_6251():
    """The oracle's AMT on an ISO case, against a hand-worked Form 6251.
    ...
    This is the burn-in for F14 on the oracle side: it flips if Tax-Calculator
    adopts the add-back.
    """
