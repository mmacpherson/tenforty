...
data BracketTable = BracketTable
...
instance ToJSON ComputationGraph where
  toJSON ComputationGraph{..} = object
    [ "meta"    .= cgMeta
    ...
    where
      ...
      nodeMapToObject m = Aeson.Object $ KM.fromList
...
instance ToJSON Op where
  toJSON = \case
    ...
    OpPhaseOut base threshold r agi -> object
      [ "type"      .= ("phase_out" :: Text)
      , "base"      .= base
      , "threshold" .= threshold
      , "rate"      .= r
    ...
    OpByStatus sn -> object
      [ "type"   .= ("by_status" :: Text)
      , "values" .= sn
      ]

instance ToJSON StatusValues where
  toJSON StatusValues{..} = object
    [ "single"           .= svSingle
...
instance ToJSON BracketTable where
  toJSON BracketTable{..} = object
    [ "brackets" .= btBrackets
    ]

instance ToJSON StatusBrackets where
  toJSON StatusBrackets{..} = object
    [ "single"           .= sbSingle
      ...
      LineWorksheet _ steps -> do
        ...
        case steps of
          ...
          _  -> do
            ...
            case mLastId of
              Just lastId -> registerLine (lineId ln) lastId
              Nothing -> pure ()

  outputIds <- forM (formOutputs frm) $ \ln -> do
...
compileBracketTable bt = BracketTable
  { btBrackets = StatusBrackets
