...
import TenForty
import qualified TenForty.Compile.JSON as JSON
...
spec n = do
  ...
  describe "E2E Compiled Form Properties" $ do
    forM_ taxYears $ \ty -> do
      ...
      it ("tax > 0 when wages exceed standard deduction (" ++ yr ++ ")") $ property $
        forAll (elements allFilingStatuses) $ \status ->
          case tyForm ty of
            Right frm ->
              let graph = compileForm frm
      ...
      it ("MFJ tax <= Single tax for same income (" ++ yr ++ ")") $
        withMaxSuccess (n `div` 2) $ property $
...
evalGraphDetailed graph status wages =
  let nodeMap = cgNodes graph
      ...
      findNodeByName name =
        case [ nodeId n | n <- Map.elems nodeMap
             , nodeName n == Just (T.pack name)
             ] of
          (nid:_) -> Just nid
          [] -> Nothing
      ...
      evalOp acc nid = \case
        OpInput -> if nodeName (nodeMap Map.! nid) == Just "L1a" then wages else 0
        OpImport _ _ _ -> 0  -- Imports from other forms treated as 0 in tests
        OpLiteral v -> v
