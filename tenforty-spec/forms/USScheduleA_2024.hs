...
usScheduleA_2024 = form "us_schedule_a" 2024 $ do
  ...
  _l4 <- keyOutput "L4" "medical_deduction" "Subtract L3 from L1 (not below 0)" $
  ...
  saltCap <- interior "SALTCap" "salt_cap" $
  ...
  _l7 <- keyOutput "L7" "total_taxes" "Add lines 5e and 6" $
    l5e .+. l6
  ...
  _l10 <- keyOutput "L10" "total_interest" "Add lines 8e and 9" $
    l8e .+. l9
  ...
  _l14 <- keyOutput "L14" "total_charity" "Add lines 11 through 13" $
    l11 .+. l12 .+. l13
  ...
  _l17 <- keyOutput "L17" "total_itemized" "Total itemized deductions" $
