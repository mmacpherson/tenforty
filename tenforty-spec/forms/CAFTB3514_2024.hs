...
caFTB3514_2024 = form "ca_ftb_3514" 2024 $ do
    ...
    l5 <-
        interior "L5" "total_earned_income" $
            l3 .+. l4
    ...
    overLimit <-
        interior "L9_check" "over_limit_check" $
    ...
    phaseInThreshold <-
        interior "L10_phasein" "phase_in_threshold" $
            ifGte
                l2
                (lit 1)
    ...
    excessIncome <-
        interior "L13_excess" "excess_income" $
            l5 `subtractNotBelowZero` phaseOutStart

    eitcPhaseOutReduction <-
        interior "L14_reduction" "phase_out_reduction" $
