...
usForm8863_2024 :: Either FormError Form
usForm8863_2024 = form "us_form_8863" 2024 $ do
  ...
  l3 <- interior "L3" "aotc_threshold" $
    byStatusE (fmap lit aotcThreshold2024)

  -- Line 4: Subtract line 3 from line 2. If zero or less, skip to line 7
  l4 <- interior "L4" "aotc_excess_magi" $
    l2 `subtractNotBelowZero` l3

  -- Line 5: Phase-out range ($10,000)
  l5 <- interior "L5" "aotc_phaseout_range" $
    lit (aotcPhaseOutRange2024)
  ...
  l8 <- interior "L8" "aotc_after_phaseout" $
    l1 .*. (rate 1.0 .-. minE (l4 ./. l5) (rate 1.0))

  -- Line 9: Refundable portion (40% of line 8)
  _l9 <- keyOutput "L9" "refundable_aotc" "Refundable American opportunity credit (to Form 1040, line 29)" $
    l8 .*. lit (aotcRefundableRate2024)

  -- Part II: Nonrefundable Education Credits

  -- Line 10: Nonrefundable portion of AOTC (60% of line 8)
  l10 <- interior "L10" "nonrefundable_aotc" $
    l8 .*. lit 0.60
  ...
  l13 <- interior "L13" "llc_threshold" $
    byStatusE (fmap lit llcThreshold2024)

  -- Line 14: Subtract line 13 from line 12
  l14 <- interior "L14" "llc_excess_magi" $
    l12 `subtractNotBelowZero` l13

  -- Line 15: Phase-out range
  l15 <- interior "L15" "llc_phaseout_range" $
    lit (llcPhaseOutRange2024)

  -- Lines 16-18: Compute LLC phase-out and apply to credit
  -- Computed inline to avoid storing Rate values as lines
  l18 <- interior "L18" "llc_after_phaseout" $
    l11 .*. (rate 1.0 .-. minE (l14 ./. l15) (rate 1.0))

  -- Line 19: Add lines 10 and 18 (total nonrefundable education credits)
  _l19 <- keyOutput "L19" "nonrefundable_education" "Nonrefundable education credits (to Schedule 3, line 3)" $
    l10 .+. l18
  ...
  _l32 <- keyOutput "L32" "llc_credit" "Lifetime learning credit (20% of expenses, max $2,000)" $
    smallerOf l31Expenses (lit (llcExpenseLimit2024)) .*. lit (llcRate2024)
