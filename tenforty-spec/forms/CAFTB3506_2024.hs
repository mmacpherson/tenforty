...
caFTB3506_2024 :: Either FormError Form
caFTB3506_2024 = form "ca_ftb_3506" 2024 $ do
    ...
    l3 <-
        interior "L3" "expense_limit" $
            ifGte
                l1
                (lit 2)
                (lit caDepCareLimit2Plus2024)
                (lit caDepCareLimit1Person2024)

    -- Line 4: Enter the smaller of line 2 or line 3
    l4 <-
        interior "L4" "limited_expenses" $
            smallerOf l2 l3
    ...
    l7 <-
        interior "L7" "earned_income_limited" $
    ...
    -- Phase-out: percentage decreases as AGI increases from $40,000 to $100,000
    ...
    excessAgi <-
        interior "L9_excess" "agi_excess" $
            l8 `subtractNotBelowZero` lit caDepCarePhaseoutStart2024

    -- Phase-out range: $100,000 - $40,000 = $60,000
    -- Rate reduction per $1,000 of excess AGI: 50% / 60 = ~0.833% per $1,000
    -- Or: excess / range * 50%
    -- We calculate steps in $1000 increments for easier math
    -- Multiply by dollars 1 to convert Rate back to Dollars for floorE
    l9Steps <-
        interior "L9_steps" "percent_reduction_steps" $
            floorE (dollars 1 .*. (excessAgi ./. dollars 1000))

    -- Each $1,000 over threshold reduces credit by ~0.833% (50% / 60 = 0.8333%)
    -- Using 60 steps from $40k to $100k
    -- Credit percent = 50% - (steps * 50/60)%
    -- = 50% - (steps * 0.8333%)
    -- Store as cents for integer math: 50 cents - (steps * 50 / 60) cents
    l9PercentCents <-
        interior "L9" "credit_percent_cents" $
            dollars 50 `subtractNotBelowZero` (l9Steps .*. rate (50.0 / 60.0))

    -- Line 10: Tentative credit (Line 7 × credit percentage)
    -- Convert cents to rate: cents / 100
    l10 <-
        interior "L10" "tentative_credit" $
    ...
    l13 <-
        interior "L13" "net_employer_benefits" $
    ...
    _l15 <-
        keyOutput "L15" "taxable_benefits" "Taxable dependent care benefits" $
