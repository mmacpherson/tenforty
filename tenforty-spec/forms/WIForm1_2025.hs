...
wiForm1_2025 :: Either FormError Form
wiForm1_2025 = form "wi_form1" 2025 $ do
    ...
    -- Lines 10-20: Wisconsin subtractions from income (from Schedule I)
    -- Simplified: using a single input for total subtractions (excluding retirement exclusion)
    l10 <- keyInput "L10" "wi_subtractions" "Total Wisconsin subtractions (from Schedule I, excluding retirement)"

    -- Line 21: Retirement income exclusion (new in 2025)
    -- Up to $24,000 per individual age 67+, $48,000 for MFJ if both 67+
    l21 <- keyInput "L21" "wi_retirement_exclusion" "Retirement income exclusion (age 67+)"

    -- Total subtractions including retirement exclusion
    l21_total <-
        interior "L21_total" "wi_total_subtractions" $
            l10 .+. l21

    -- Line 22: Wisconsin adjusted gross income (WAGI)
    l22 <-
        keyOutput "L22" "wi_agi" "Wisconsin adjusted gross income" $
            l9 `subtractNotBelowZero` l21_total
    ...
    l40 <-
        keyOutput "L40" "wi_bracket_tax" "Tax from Wisconsin tax rate schedule" $
            bracketTax "wi_brackets_2025" l39
