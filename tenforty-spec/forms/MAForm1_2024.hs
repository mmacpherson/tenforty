...
maForm1_2024 :: Either FormError Form
maForm1_2024 = form "ma_1" 2024 $ do
    ...
    l2a <-
        interior "L2a" "personal_exemption" $
            byStatusE (fmap lit maPersonalExemption2024)
    ...
    l23a_tax <-
        interior "L23a_tax" "short_term_gains_tax" $
            l23a .*. rate maShortTermCapitalGainsRate2024
    ...
    l23b_tax <-
        interior "L23b_tax" "collectibles_tax" $
            l23b .*. rate maLongTermCollectiblesRate2024
    ...
    surtaxableIncome <-
        interior "surtaxable_income" "surtaxable_income" $
            totalTaxableForSurtax `subtractNotBelowZero` lit maSurtaxThreshold2024

    l28b <-
        keyOutput "L28b" "ma_surtax" "4% surtax" $
            surtaxableIncome .*. rate maSurtaxRate2024
