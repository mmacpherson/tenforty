...
mdForm502_2024 :: Either FormError Form
mdForm502_2024 = form "md_502" 2024 $ do
    defineTable marylandBracketsSingleTable2024
    ...
    -- Standard deduction is 15% of MD AGI, min $1,800, max $2,700
    -- Maryland has two different bracket schedules, so deduction varies by status
    itemized <- keyInput "L17_itemized" "md_itemized" "Maryland itemized deductions"

    stdDed15Pct <-
        interior "StdDed15Pct" "std_ded_15_percent" $
            l16 .*. rate mdStandardDeductionRate2024

    stdDedClamped <-
        interior "StdDedClamped" "std_ded_clamped" $
            smallerOf (greaterOf stdDed15Pct (lit mdStandardDeductionMin2024)) (lit mdStandardDeductionMax2024)

    l17 <- interior "L17" "deduction" $ greaterOf itemized stdDedClamped
    ...
    -- We select the bracket table based on filing status
    taxSingle <-
        interior "TaxSingle" "tax_schedule_i" $
            bracketTax "md_brackets_single_2024" l20

    taxJoint <-
        interior "TaxJoint" "tax_schedule_ii" $
            bracketTax "md_brackets_joint_2024" l20
