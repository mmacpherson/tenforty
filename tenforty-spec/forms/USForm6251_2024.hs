...
usForm6251_2024 :: Either FormError Form
usForm6251_2024 = form "us_form_6251" 2024 $ do
  ...
  l4 <- keyOutput "L4" "amti" "Alternative minimum taxable income" $
    l1 .+. l2a .+. l2b .+. l2c .-. l2d .+. l2e .+. l2f .+. l2g .-. l2h .+.
  ...
  l5Exemption <- interior "L5_exemption" "amt_exemption_base" $
    byStatusE (fmap lit amtExemption2024)

  l5Threshold <- interior "L5_threshold" "amt_phaseout_threshold" $
    byStatusE (fmap lit amtPhaseOutThreshold2024)

  -- Exemption phase-out: reduce exemption by 25% of AMTI over threshold
  l5Excess <- interior "L5_excess" "amt_phaseout_excess" $
    l4 `subtractNotBelowZero` l5Threshold

  l5Reduction <- interior "L5_reduction" "amt_exemption_reduction" $
    l5Excess .*. lit 0.25

  l5 <- interior "L5" "amt_exemption" $
    l5Exemption `subtractNotBelowZero` l5Reduction

  -- Line 6: Subtract line 5 from line 4. If zero or less, enter -0-
  l6 <- interior "L6" "amt_taxable" $
    l4 `subtractNotBelowZero` l5

  -- Line 7: AMT rate threshold (26% up to this amount, 28% above)
  l7Threshold <- interior "L7_threshold" "amt_rate_threshold" $
    byStatusE (fmap lit amtRate1Threshold2024)
  ...
  -- Amount taxed at 26%
  l7a <- interior "L7a" "amt_26_taxable" $
    smallerOf l6 l7Threshold

  l7b <- interior "L7b" "amt_26_tax" $
    l7a .*. lit 0.26

  -- Amount taxed at 28%
  l7c <- interior "L7c" "amt_28_taxable" $
    l6 `subtractNotBelowZero` l7Threshold

  l7d <- interior "L7d" "amt_28_tax" $
    l7c .*. lit 0.28

  -- Line 7: Total tentative minimum tax (before capital gains adjustment)
  l7 <- interior "L7" "tentative_min_tax_before_cg" $
    l7b .+. l7d
  ...
  l9 <- interior "L9" "tentative_min_tax" $
  ...
  _l11 <- keyOutput "L11" "amt" "Alternative minimum tax" $
