...
idForm40_2025 = form "id_form40" 2025 $ do
    ...
    l16_std <-
        interior "L16_std" "standard_deduction" $
            byStatusE (fmap lit idStandardDeduction2025)
    l16 <-
    ...
    -- Threshold varies by filing status: Single $4,811, MFJ/HoH/QW $9,622
    -- Rate: 5.3% (reduced from 5.695% in 2024)
    l20_threshold <-
        interior "L20_threshold" "tax_threshold" $
            byStatusE (fmap lit idTaxThreshold2025)
    l20 <-
        keyOutput "L20" "id_tax" "Idaho income tax" $
            (l19 `subtractNotBelowZero` l20_threshold) .*. rate idTaxRate2025
