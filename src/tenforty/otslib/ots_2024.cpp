...
namespace taxsolve_NJ_1040_2024 {
#define SINGLE 		        1
...
#define WIDOW		        5
/************************************************************************/
/* TaxSolve_NJ_1040_2024.c - 						*/
/* Copyright (C) 2025 - Aston Roberts					*/
/* 									*/
/* Compile:   gcc taxsolve_NJ_1040_2024.c -o taxsolve_NJ_1040_2024	*/
/* Run:	      ./taxsolve_NJ_1040_2024  NJ_1040_2024.txt 		*/
/* 									*/
/* GNU Public License - GPL:						*/
/* This program is free software; you can redistribute it and/or	*/
/* modify it under the terms of the GNU General Public License as	*/
...
/* 02111-1307 USA							*/
/* 									*/
/* Aston Roberts 1-3-2025	aston_roberts@yahoo.com			*/
/************************************************************************/
...
*/
 ...
 check_form_version( word, "Title:  NJ-1040 State 2024" );
...
#define No  0
/************************************************************************
* TaxSolve_US_1040_Sched_E_brokerage_royalties_2024.c -
*
* E. Strnod 9/12/2024
*************************************************************************/
/*
 ...
 */
 ...
 now = time(0);
 fprintf(outfile,"\n%s,  v%2.2f, %s\n", word, thisversion, ctime( &now ));
 check_form_version( word, "Title: 2024 Form 8829" );
 ...
 */
 ...
 now = time(0);
 fprintf(outfile,"\n%s,  v%2.2f, %s\n", word, thisversion, ctime( &now ));
 check_form_version( word, "Title: 2024 Form 8959" );

 /* ----- Accept form data and process the numbers.         ------ */
 /* ----- Place all your form-specific code below here .... ------ */

 // fprintf(outfile,"\n--- THIS IS PRELIMINARY USER-CONTRIBUTED FORM ---\n");
 // fprintf(outfile,"--- NOT YET FULLY UPDATED FOR 2024. ---\n\n");

 // MarkupPDF( 1, 240, 40, 17, 1.0, 0, 0 ) NotReady "This program is NOT updated for 2024."
 // add_pdf_markup( "NotReady", 1, 240, 40, 17, 1, 1.0, 0, 0, "\"This program is NOT ready for 2024.\"" );
 ...
 */
 ...
 now = time(0);
 fprintf(outfile,"\n%s,  v%2.2f, %s\n", word, thisversion, ctime( &now ));
 check_form_version( word, "Title: 2024 Form 8960" );
...
namespace taxsolve_US_1040_Sched_SE_2024 {
#define Yes 1
#define No  0
/************************************************************************
...
int main( int argc, char *argv[] )
{
 ...
 /* Accept parameters from input file. */
 /* Expect lines, something like:
        Title:  1040 Schedule SE - 2024 Return
        L2              {Net Profit/Loss}
        . . .
 */
 ...
 now = time(0);
 fprintf(outfile,"\n%s,  v%2.2f, %s\n", word, thisversion, ctime( &now ));
 check_form_version( word, "Title:  1040 Schedule SE - 2024" );
...
int main(int argc, char *argv[])
{
  ...
  */
  ...
  now = time(0);
  fprintf(outfile, "\n%s,  v%2.2f, %s\n", word, thisversion, ctime(&now));
  check_form_version( word, "Title: Form 8606 for tax-year 2024" );
    ...
    if (L[21] > 0 && L[23] > 0) {
      ...
      if (L25a > 0) {
        showline_wlabelmsg("L25b", L25b,
                           "Also, enter this amount on 2024 Form 8915-C, line "
                           "23, or 2024 Form 8915-D, line 14, as applicable");

        L25c = L25a - L25b;
        if (L25c > 0) {
          showline_wlabelmsg(
              "L25c", L25c,
              "Also include this amount on 2024 Form 1040 or 1040-SR, line 4b; "
              "or 2024 Form 1040-NR, line 16b.");
...
/* The following two tax functions copied from taxsolve_CA_540_2024.c. */
...
double		/* Reoccuring threshold values. */	/* Not updated for 2024. */
	thresh_sep_single =	237035.0,
	thresh_mfj =		474075.0,
	thresh_HoH =		355558.0;

double		/* Other recurring values */		/* Not updated for 2024 */
 ...
 */
 ...
 now = time(0);
 fprintf(outfile,"\n%s,  v%2.2f, %s\n", word, thisversion, ctime( &now ));
 check_form_version( word, "Title:  Form 5805 for Tax Year 2024" );

 #if (1)
   add_pdf_markup( "NotReady", 1, 240, 40, 17, 1, 1.0, 0, 0, "\"This program is NOT ready for 2024.\"" );
   fprintf(outfile, "This program is NOT ready for 2024.\n\n");
   #ifdef microsoft
    system( "start bin\\notify_popup -delay 3 -expire 10 \"Warning: This program is NOT ready for 2024.\"" );
   #else
    system( "bin/notify_popup -delay 3 -expire 10 \"Warning: This program is NOT ready for 2024.\" &" );
...
/* 02111-1307 USA							*/
/* 									*/
/* Updated/checked for 2024 tax year.					*/
/************************************************************************/
...
int main( int argc, char *argv[] )
{
 ...
 printf("Form 8889 HSA, 2024 - v%3.2f\n", thisversion );
 ...
 */
 ...
 now = time(0);
 fprintf(outfile,"\n%s,	 v%2.2f, %s\n", word, thisversion, ctime( &now ));
 check_form_version( word, "Title: 8889 HSA Form - 2024" );
...
#define No  0
/************************************************************************/
/* taxsolve_ma_1_2024.c - OpenTaxSolver for Mass Form 1 		*/
/* Copyright (C) 2025 							*/
/* 									*/
/* OTS Project Home Page and Updates:  					*/
/*		http://opentaxsolver.sourceforge.com/			*/
/* 									*/
/* Compile:   cc taxsolve_ma_1_2024.c -o taxsolve_ma_1_2024		*/
/* Run:       ./taxsolve_ma_1_2024  Mass1_2024.txt			*/
/* 									*/
/* GNU Public License - GPL:						*/
/* This program is free software; you can redistribute it and/or	*/
/* modify it under the terms of the GNU General Public License as	*/
...
int main( int argc, char *argv[] )
{
 ...
 printf("Massachusetts Form-1 2024 - v%3.2f\n", thisversion);
 ...
 */
 ...
 now = time(0);
 fprintf(outfile,"\n%s,	 v%2.2f, %s\n", word, thisversion, ctime( &now ));
 check_form_version( word, "Title:  Massachusetts Form 1 Tax Form - 2024" );
 ...
 GetLine( "L39", &L[39] );	/* Last year's overpayment you want applied to 2024 estimated tax */
 ShowLineNonZero(39);

 GetLine( "L40", &L[40] );	/* 2024 estimated tax payments */
 ShowLineNonZero(40);
...
namespace taxsolve_NY_IT201_2024 {
#define SINGLE                  1
#define MARRIED_FILING_JOINTLY	2
#define MARRIED_FILING_SEPARAT 	3
#define HEAD_OF_HOUSEHOLD       4
...
#define False 0
/************************************************************************/
/* TaxSolve_NY_IT-201_2024.c - NY State Tax form IT-201 for 2024.	*/
/* Copyright (C) 2003-2025 - Aston Roberts, Skeet Monker		*/
/* 									*/
/* Compile:   gcc taxsolve_NY_IT201_2024.c -o taxsolve_NY_IT201_2024	*/
/* Run:	      ./taxsolve_NY_IT201_2024  NY_IT201_2024.txt 		*/
/* 									*/
/* GNU Public License - GPL:						*/
/* This program is free software; you can redistribute it and/or	*/
/* modify it under the terms of the GNU General Public License as	*/
...
*/
 ...
 now = time(0);
 fprintf(outfile,"\n%s,	 v%2.2f, %s\n", word, thisversion, ctime( &now ));
 check_form_version( word, "Title:  NY State 2024" );
...
namespace taxsolve_PA_40_2024 {
#define thisversion 22.00
#define SINGLE 		        1
#define MARRIED_FILING_JOINTLY  2
#define MARRIED_FILING_SEPARAT  3
#define WIDOW		        1
...
#define No  0
/************************************************************************/
/* TaxSolve_PA40_2024.c - Pennsylvania 2024 PA-40 State Tax Form.	*/
/* Copyright (C) 2025, - Aston Roberts					*/
/* 									*/
/* Compile:   gcc taxsolve_PA40_2024.c -o taxsolve_PA40_2024		*/
/* Run:	      ./taxsolve_PA40_2024  PA40_2024.txt 			*/
/* 									*/
/* GNU Public License - GPL:						*/
/* This program is free software; you can redistribute it and/or	*/
/* modify it under the terms of the GNU General Public License as	*/
 ...
 */
 ...
 now = time(0);
 fprintf(outfile,"\n%s,	 v%2.2f, %s\n", word, thisversion, ctime( &now ));
 check_form_version( word, "Title:  PA State Tax Form PA-40 for 2024" );
 ...
 GetLineF( "L15", &L[15] );	/* 2024 Estimated Installment payments. */

 GetLineF( "L16", &L[16] );	/* 2024 Extension payment. */
...
#define No  0
/************************************************************************/
/* TaxSolve_usa_fed1040_2024.c - OpenTaxSolver for USFed1040 		*/
/* Copyright (C) 2025 - Aston Roberts					*/
/* 									*/
/* Tax Solver for US Fedral 1040 Income Tax return for 2024 Tax Year.	*/
/* 									*/
/* OTS Project Home Page and Updates:  					*/
/*		http://opentaxsolver.sourceforge.com/			*/
/* 									*/
/* Compile:   cc taxsolve_US_1040_2024.c -o taxsolve_US_1040_2024       */
/* Run:       ./taxsolve_US_1040_2024  Fed1040_2024.txt                 */
/* 									*/
/* GNU Public License - GPL:						*/
/* This program is free software; you can redistribute it and/or	*/
/* modify it under the terms of the GNU General Public License as	*/
...
double Sched2_1a=0.0, Sched2_1b=0.0, Sched2_1c=0.0, Sched2_1d=0.0, Sched2_1e=0.0, Sched2_1f=0.0, Sched2_1y=0.0, Sched2_1z=0.0;
...
/*  From page 36 of main instructions (i1040gi.pdf).		   */
/*-----------------------------------------------------------------*/
void capgains_qualdividends_worksheets( int status )			/* Updated for 2024. */
{
 int j;
 ...
 switch (status)
  {
   case SINGLE: case MARRIED_FILING_SEPARAT: qcgws[6] = 47025.0; break;
   case MARRIED_FILING_JOINTLY: case WIDOW:  qcgws[6] = 94050.0; break;
   case HEAD_OF_HOUSEHOLD: 		      qcgws[6] = 63000.0; break;
  }
 ...
 switch (status)
  {
   case SINGLE:  			      qcgws[13] = 518900.0;  break;
   case MARRIED_FILING_SEPARAT:	      	      qcgws[13] = 291850.0;  break;
   case MARRIED_FILING_JOINTLY: case WIDOW:   qcgws[13] = 583750.0;  break;
   case HEAD_OF_HOUSEHOLD: 		      qcgws[13] = 551350.0;  break;
  }
...
/*----------------------------------------------------------------------------------------------*/
double form6251_AlternativeMinimumTax( int itemized )						/* Updated for 2024. */
{
 ...
 switch (status)
  {
	...
	thresholdA = 1218700.0;
	thresholdB = 1751900.0;
	thresholdC = 232600.0;
	offsetA = 4652.0;
	amtexmption = 133300.0;
	break;
     case MARRIED_FILING_SEPARAT:
	thresholdA = 609350.0;
	thresholdB = 875950.0;
 ...
 if (amtws[6] > 0.0)
  { /* AMT Lines 7 through 9, */
	...
	amtws[20] = NotLessThanZero( L[15] );
       ...
       switch (status)
	{
	   case SINGLE:  			      amtws[25] = 518900.0;  break;
	   case MARRIED_FILING_SEPARAT:	      	      amtws[25] = 291850.0;  break;
	   case MARRIED_FILING_JOINTLY: case WIDOW:   amtws[25] = 583750.0;  break;
	   case HEAD_OF_HOUSEHOLD: 		      amtws[25] = 551350.0;  break;
	   default:  printf("Status %d not handled.\n", status);  exit(1);
	}
       amtws[26] = amtws[21];
       if (Do_QDCGTW)
...
void CapitalLossCarryOverWorksheet( char *fedlogfile, struct FedReturnData *LastYearsReturn )	/* Updated for 2024. */
...
/*   buy_amnt   date 							*/
/*   sell_amnt  date 							*/
/*   adj_code, adj_amnt		(Can default to ~ ~.)			*/
/*									*/
/************************************************************************/
void get_cap_gains()		/* This is Schedule-D. */			/* Updated for 2024. */
{
...
/*--------------------------------------------------------*/
void sched_D_tax_worksheet( int status )			/* Updated for 2024. */
{
 double ws[100];
 int k;
 ...
 switch (status)
  { case SINGLE: case MARRIED_FILING_SEPARAT: ws[15] = 47025.0; break;
    case MARRIED_FILING_JOINTLY: case WIDOW:  ws[15] = 94050.0; break;
    case HEAD_OF_HOUSEHOLD:      	      ws[15] = 63000.0; break;
  }
 ws[16] = smallerof( ws[1], ws[15] );
 ws[17] = smallerof( ws[14], ws[16] );
 ws[18] = NotLessThanZero( ws[1] - ws[10] );
 switch (status)
  { case SINGLE: case MARRIED_FILING_SEPARAT: ws[19] = smallerof( ws[1], 191950.0 );  break;
    case MARRIED_FILING_JOINTLY: case WIDOW:  ws[19] = smallerof( ws[1], 383900.0 );  break;
 ...
 if (ws[1] != ws[16])
  { /*lines23-43*/
   ...
   switch (status)
    { case SINGLE: 			ws[26] = 518900.0;  break;
      case MARRIED_FILING_SEPARAT: 	ws[26] = 291850.0;  break;
      case MARRIED_FILING_JOINTLY:
      case WIDOW:  			ws[26] = 583750.0;  break;
      case HEAD_OF_HOUSEHOLD:		ws[26] = 551350.0;  break;
    }
...
/*-------------------------------------------------------------*/
/* Social Security Worksheet - From main Instructions page 32. */
/*-------------------------------------------------------------*/
void SocSec_Worksheet()							/* Updated for 2024. */
{	/* Depends on Sched1, lines 11-25, and 1040 lines 1-8. Sets L[6]. */
 ...
 if (status == MARRIED_FILING_JOINTLY)
  ws[10] = 12000.0;      						/* Updated for 2024. */
 else
  ws[10] = 9000.0;
...
int main( int argc, char *argv[] )						/* Updated for 2024. */
{
 ...
 /* Expect  US-Fed-1040 lines, something like:
	Title:  Federal 1040 2024 Return
	L1		{Wages}
	L2b		{Interest}
	L3b		{Dividends}
 */
 ...
 now = time(0);
 fprintf(outfile,"\n%s,	 v%2.2f, %s\n", word, thisversion, ctime( &now ) );
 check_form_version( word, "Title:  US Federal 1040 Tax Form - 2024" );
 ...
 L[25] = L25a + L25b + L25c;
 GetLine( "L26", &L[26] );	/* Estimated tax payments for 2024. */
 ...
 if (StdDedChart_NumBoxesChecked == 0)
  {
   S_STD_DEDUC   = 14600.0;						/* Updated for 2024. */
   MFJ_STD_DEDUC = 29200.0;
   MFS_STD_DEDUC = 14600.0;
   HH_STD_DEDUC  = 21900.0;
  }
 else
  { /* Std. Deduction chart for People who were Born Before January 2, 1960, or Were Blind, pg 34. */
    switch (StdDedChart_NumBoxesChecked)		/* Does not handle if someone claims you or joint-spouse as dependent. */
     {				/* (Qualifying Widow/er has same amounts as MFJ, so not broken into separate variable.) */
      case 1:
	S_STD_DEDUC   = 16550.0;					/* Updated for 2024. */
	MFJ_STD_DEDUC = 30750.0;
	MFS_STD_DEDUC = 16150.0;
	HH_STD_DEDUC  = 23850.0;
	break;
      case 2:
	S_STD_DEDUC   = 18500.0;
	MFJ_STD_DEDUC = 32300.0;
	MFS_STD_DEDUC = 17700.0;
	HH_STD_DEDUC  = 25800.0;
	break;
      case 3:
	MFJ_STD_DEDUC = 33850.0;
	MFS_STD_DEDUC = 19250.0;
	S_STD_DEDUC   = 18500.0;	/* Cannot happen, but set to appease compiler. */
	HH_STD_DEDUC  = 25800.0;	/* .. */
	break;
      case 4:
	MFJ_STD_DEDUC = 35400.0;
	MFS_STD_DEDUC = 20800.0;
	S_STD_DEDUC   = 18500.0;	/* Cannot happen, but set to appease compiler. */
	HH_STD_DEDUC  = 25800.0;	/* .. */
	break;
 ...
 GetLineF( "S1_1099K_err", &S1_1099K_err );	/* Amounts on 1099-K forms reported in error or for personal loss. */
 ...
 Sched1[9] = Sched1[9] + Sched1[8];
 GetLineFnz( "S1_8v", &Sched1[8] );	/* Digital assets received as ordinary income */
 Sched1[9] = Sched1[9] + Sched1[8];
 ...
 /* Look for optional Force-Itemize line.  (Remove *optional* logic for 2024, once A18 is on ALL templates.) */
 ...
 switch (status)	/* Check for minimum income to file. (min2file) */		/* Updated for 2024. */
  {			/* Listed in Instructions page-9, in Chart A - For Most People. */
   case SINGLE:  		  if (under65) exemption_threshold = 14600.0;
				  else  exemption_threshold = 16550.0;
	break;
   case MARRIED_FILING_JOINTLY:	  if (under65==2) exemption_threshold = 29200.0;
				  else
				  if (under65==1) exemption_threshold = 30750.0;
				  else  exemption_threshold = 32300.0;
				  if (under65 != 2) over65 = 1;
	break;
   case MARRIED_FILING_SEPARAT:  exemption_threshold = 5.0;
	break;
   case HEAD_OF_HOUSEHOLD: 	  if (under65) exemption_threshold = 21900.0;
				  else  exemption_threshold = 23850.0;
	break;
   case WIDOW:  		  if (under65) exemption_threshold = 29200.0;
				  else  exemption_threshold = 30750.0;
  }
 ...
 GetLine( "S2_1a", &Sched2_1a );	/* Excess advance premium tax credit repayment. Form 8962. */
					/* (Needed by AMT form6251.) */
 GetLine( "S2_1b", &Sched2_1b );	/* Repayment of new clean vehicle credit(s) transferred to dealer. */
 GetLine( "S2_1c", &Sched2_1c );	/* Repayment of previously owned clean vehicle credit(s) transferred to dealer. */
 GetLine( "S2_1d", &Sched2_1d );	/* Recapture of net EPE from Form 4255, line 2a, column (l). */
 GetLine( "S2_1e", &Sched2_1e );	/* Excessive payments (EP) from Form 4255. */
 GetLine( "S2_1f", &Sched2_1f );	/* 20% EP from Form 4255. */
 GetLine( "S2_1y", &Sched2_1y );	/* Other additions to tax (see instructions). */

 Sched2_1z = Sched2_1a + Sched2_1b + Sched2_1c + Sched2_1d + Sched2_1e + Sched2_1f + Sched2_1y;
 Sched2[1] = Sched2_1z;
 ...
 Sched2[2] = form6251_AlternativeMinimumTax( itemize );		/* (Depends on Sched2_1a & Sched3[1].) */
 if (Sched2[2] == 0.0)
  fprintf(outfile," (Not subject to Alternative Minimum Tax.)\n");
 else
  fprintf(outfile," (You must pay Alternative Minimum Tax.)\n");

 showline_wlabelnz( "S2_1a", Sched2_1a );
 showline_wlabelnz( "S2_1b", Sched2_1b );
 showline_wlabelnz( "S2_1c", Sched2_1c );
 showline_wlabelnz( "S2_1d", Sched2_1d );
 showline_wlabelnz( "S2_1e", Sched2_1e );
 showline_wlabelnz( "S2_1f", Sched2_1f );
 showline_wlabelnz( "S2_1y", Sched2_1y );
 showline_wlabelnz( "S2_1z", Sched2_1z );
 showline_wlabelmsg_nz( "S2_2", Sched2[2], "   Alternative Minimum Tax" );
...
namespace taxsolve_VA_760_2024 {
#define SINGLE 		        1
#define MARRIED_FILING_JOINTLY  2
#define MARRIED_FILING_SEPARAT  3
#define HEAD_OF_HOUSEHOLD       4
#define WIDOW		        5
/************************************************************************/
/* TaxSolve_VA760_2024.c - 						*/
/* Copyright (C) 2025 - Aston Roberts					*/
/* 									*/
/* Compile:   gcc taxsolve_VA760_2024.c -o taxsolve_VA760_2024		*/
/* Run:	      ./taxsolve_VA760_2024  VA_760_2024.txt 			*/
/* 									*/
/* GNU Public License - GPL:						*/
/* This program is free software; you can redistribute it and/or	*/
/* modify it under the terms of the GNU General Public License as	*/
...
*/
 ...
 now = time(0);
 fprintf(outfile,"\n%s,	 v%2.2f, %s\n", word, thisversion, ctime( &now ));
 check_form_version( word, "Title:  VA-760 State 2024" );
 ...
 if (yourDOB.year < 1960)			/* Updated for 2024. */
  {
 ...
 if (status == MARRIED_FILING_JOINTLY)
  {
   if (spouseDOB.year < 1960)			/* Updated for 2024. */
    {
 ...
 GetLineF( "L4", &L[4] );	/* Deduction for age on Jan 1, 2024. */
...
*/
 ...
 GetLineF( "L19a", &L[19] );	/* Virginia tax withheld for 2024. */
 GetLineF( "L19b", &L19b );	/* Spouse's Virginia tax withheld. */

 GetLineF( "L20", &L[20] );	/* Estimated tax paid for 2024. (form 760ES) */

 GetLineF( "L21", &L[21] );	/* Amount of last year's overpayment applied toward 2024 estimated tax. */
...
/* 2024 Tax Rate Schedule for Estates/Trusts - From 2024 Form 1041 Instructions, page 30 */
 ...
 */
 ...
 now = time(0);
 fprintf(outfile,"\n%s,  v%2.2f, %s\n", word, thisversion, ctime( &now ));
 check_form_version( word, "Title:  Form 2210 for Tax Year 2024" );
