...
static int round_to_whole_dollars=0;	/* Option to use whole-dollars. */
static int value_was_detected=0;
...
inline char *mystrcasestr( char *haystack, char *needle )
{
 int j=0;
 char *hs, *ndl, *pt;
 hs = strdup( haystack );
...
/***************************************************************************/
inline void next_csv( char *line, char *word, char delim )
...
inline char *format_mmddyyyy( int month, int day, int year )
{
...
inline void read_line_safe( FILE *infile, char *line, int maxlen )
{
 char ch;
 int j=0;
 do  
...
inline void showline_wrksht_nz( char *wrksht, int j, double *x )
...
inline void showline_wlabelmsg_nz( char *label, double value, char *msg )
{ 
 if (value != 0.0)
  fprintf(outfile, "%s = %6.2f\t\t%s\n", label, value, msg );
}
...
inline void Show_String_wLabel( char *label, char *msg )
{ 
 if ((msg != 0) && (msg[0] != '\0'))
  fprintf(outfile, "%s %s\n", label, msg );
}
...
inline void GetLineString( char *linename, char *chstr )	/* Get a character-string value. */
{
 char word[1024];
 get_parameter( infile, 's', word, linename);
...
inline void capitalize( char *word )
{
 int k=0;
 while (word[k] != '\0')
  {
...
/*------------------------------------------------------------------------------*/
inline char *GetTextLineF( char *linename )
{
 int k=0;
 char line[5000];
...
inline char *GetTextLine( char *linename )
{
 int priorstate;
 char *chstr;
...
inline int whitespace_invariant_strstr( char *haystack, char *needle )	/* Return 1 if match, otherwise 0 if mismatch. */
{
 int ret;
...
inline int check_form_version( char *title_as_read_in, char *expected_title )
...
inline void check_if_yes( char *label )	/* Check box 'Y' or "N'. */
{
 char word[999];
 int flag;
...
inline void GrabLineValue( char *label, char *fline, double *value )
{
 char twrd[2048];
...
inline void GrabLineString( char *fline, char *strng )
...
inline void GrabLineAlloc( char *fline, char **strng )
...
inline void ConvertSlashes( char *fname )
...
inline int ImportReturnData(char *return_filename, P_FORM_IMPORT_DEF p_form_imp_def, int num_imp_defs)
{
...
inline void add_pdf_markup( char *tagname, int page, float xpos, float ypos, 
